...
- **Double-Pocket Adapter Design**:
  - The glass focusing adapter uses a concentric double-pocket layout on the front face: an outer pocket ($70\text{ mm} \times 70\text{ mm}$ at $0.3\text{ mm}$ depth) to seat the pressure plate, and an inner pocket ($60.5\text{ mm} \times 60.5\text{ mm}$ at $2.3\text{ mm}$ depth) to hold the focusing glass.
- **Flush M2 Captured Nut Channels**:
  - Pockets on the camera-facing back face (`<Z`) house captured M2 nuts (width $4.3\text{ mm}$, depth $3.5\text{ mm}$), creating a durable metal-threaded connection that sits recessed for easy assembly and provides a tolerance buffer for M2 x 12 mm screws.
...
- **Calibrated Snug Fit & Infinity Focus**:
  - The adapter sliding edges are **$6.5\text{ mm}$** thick (`plate_thickness`) to fit securely in the camera mount.
  - The overall center portion of the body is **$10.0\text{ mm}$** thick (`body_thickness`) to accommodate the deep film plane recess.
  - The registration plane sits at a distance of **$7.7\text{ mm}$** from the camera-facing back face, aligning the ground glass/paper surface precisely with the camera's film plane for perfect infinity focus at the lens's infinity stop.
...
- **Hardware Needed**: 
  ...
  - 4x **M2 x 12 mm** countersunk head screws. The adapter body uses clearance holes, and when coupled with the $4.0\text{ mm}$ thick pressure plate, the tip of the $12\text{ mm}$ screw remains recessed by $0.1\text{ mm}$ from the back face, preventing scratches to your camera.
...
- **Assembly**:
  1. Insert the M2 nuts into the deep pockets on the back of the adapter body.
